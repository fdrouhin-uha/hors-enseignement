    ...
    try:
        ...
        for index, row in df_code.iterrows():
            ...
            if isinstance(code_etape,str):
                ...
                if 'FA' not in intitule_code_etape:
        ...
        data.extend([
            {'Référentiel': "Loi ORE : mettre 1 si l'enseignant a étudié des dossiers, 0 sinon", 'Code étape': "variable a faire manuellement ?", 'Nombre d\'heures équivalent TD': 0.5, 'Remarque': "h pour relecture du dossier"}
            ])
        ...
        for index, row in nouveau_df.iterrows():
            ...
            if isinstance(referenciel,str):
                if referenciel not in data_ens:
                    data_ens[referenciel] = {}
                for index, row_ens in df_ens.iterrows():
                    enseignant = row_ens['NOM'] + ' ' + row_ens['Prénom']
                    if isinstance(enseignant, str):
                        if enseignant not in data_ens[referenciel]:
                            data_ens[referenciel][enseignant] = ""
        ...
        nouveau_df_ens.to_excel(nom_fich_ens, index=False)
